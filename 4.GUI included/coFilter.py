...
# rl = ['갱년기','임신부']
# healthCoFilter(loadCsv(),loadHealthCsv(),['감기','갱년기'])
# print(healthCoFilter(loadCsv(),loadHealthCsv(),rl))


'''
class MainApp(wx.Frame): ##얘는 전체 메인화면 하면 되겠음
    def __init__(self, parent):
        # wx.MainFrame.__init__(self, parent)
        fr = wx.Frame(parent=None, title="음식 추천 순위", size=(320, 200))
        fr.pnl = wx.Panel(fr)
        matchlist = (healthCoFilter(loadCsv(), loadHealthCsv(), rl))
        ch = 0
        gap = -20
